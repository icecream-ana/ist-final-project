...
# transformers==4.23.1 只有 CLIPFeatureExtractor, 但 diffusers==0.11.1 需要 CLIPImageProcessor (同一个类, 只是高版本改名了)
import transformers
if not hasattr(transformers, 'CLIPImageProcessor'):
    transformers.CLIPImageProcessor = transformers.CLIPFeatureExtractor
    print('[patch] CLIPImageProcessor -> CLIPFeatureExtractor done')
...
def main(args):
    ...
    if args.with_tracking:
        wandb.init(project='diffusion_watermark', name=args.run_name, tags=['tree_ring_watermark'], mode='offline')
    ...
    if args.with_tracking:
        ...
        wandb.log({'clip_score_mean': mean(clip_scores), 'clip_score_std': stdev(clip_scores) if len(clip_scores) > 1 else 0,
                   'w_clip_score_mean': mean(clip_scores_w), 'w_clip_score_std': stdev(clip_scores_w) if len(clip_scores_w) > 1 else 0,
