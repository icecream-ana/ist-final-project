...
def main(args):
    ...
    # create a temp ground-truth folder with only the images matching the generated range
    gt_subset_dir = tempfile.mkdtemp(prefix='fid_gt_subset_')
    for i in range(args.start, args.end):
        image_file_name = image_files[i]['file_name']
        gt_src = os.path.join(args.gt_folder, image_file_name)
        if os.path.exists(gt_src):
            shutil.copy(gt_src, os.path.join(gt_subset_dir, image_file_name))

    # fid for no_w
    if args.run_no_w:
        fid_value_no_w = calculate_fid_given_paths([gt_subset_dir, no_w_dir],
                                            50,
                                            device,
                                            2048,
                                            num_workers)
    ...
    # fid for w
    fid_value_w = calculate_fid_given_paths([gt_subset_dir, w_dir],
                                          50,
                                          device,
                                          2048,
                                          num_workers)
    ...
    shutil.rmtree(gt_subset_dir, ignore_errors=True)
